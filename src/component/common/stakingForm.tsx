...
const STAKING_CONTRACT_ADDRESS = "0xEcC30dCBEe72C3E376108145584708d9227769C7";
...
const StakingForm = () => {
  ...
  const [boostNFTNumberForDeposit, setBoostNFTNumberForDeposit] =
    useState<number>(1);
  ...
  const handleDeposit = async () => {
    if (!isConnected || chainId !== 59144 || web3 === null) return;
    ...
    try {
      let hash = await writeContractAsync({
        ...
        functionName: "stake",
        args: [
          BigInt(depositAmount) * BigInt(10 ** WOLF_TOKEN_DECIMAL),
          BigInt(stakingDuration),
        ],
      });
  ...
  return (
    <>
      <div className="flex flex-row justify-between">
        ...
        <p>
          <input type="checkbox" onClick={() => setIsNFT(!isNFT)} />{" "}
          <span className="cursor-pointer" onClick={() => setIsNFT(!isNFT)}>
            NFT staikng
          </span>
        </p>
      </div>
      <div className="flex md:flex-row flex-col py-5">
        <div className="md:w-1/2 w-full bg-white py-4 bg-opacity-[5%] mx-3 rounded-2xl  px-5">
          <div className="flex text-[27px] text-white justify-between text-center rounded-lg items-center ">
            <select
              className="bg-transparent w-[70%] py-3 text-center rounded-lg focus:border-gray-400 border-gray-500 border  outline-none "
              value={stakingDuration}
              onChange={(e) => {
                setStakingDuration(Number(e.target.value));
              }}
            >
              <option className="bg-[#0e1238] py-2 bg-opacity-90" value={1}>
                1 month
              </option>
              <option className="bg-[#0e1238] py-2 bg-opacity-90" value={3}>
                3 month
              </option>
              <option className="bg-[#0e1238] py-2 bg-opacity-90" value={6}>
                Half year
              </option>
              <option className="bg-[#0e1238] py-2 bg-opacity-90" value={12}>
                One year
              </option>
            </select>
            <Image
          ...
          <div className="mt-3 grid px-0">
            {isNFT ? (
              <select
                className="bg-transparent w-full  py-3 text-center rounded-lg focus:border-gray-400 border-gray-500 border  outline-none "
                value={boostNFTNumberForDeposit}
                onChange={(e) => {
                  setBoostNFTNumberForDeposit(Number(e.target.value));
                }}
              >
                <option className="bg-[#0e1238] py-2 bg-opacity-90" value={1}>
                  Boost NFT 1
                </option>
                <option className="bg-[#0e1238] py-2 bg-opacity-90" value={2}>
                  Boost NFT 2
                </option>
              </select>
            ) : (
              <div className="flex">
                ...
                ></input>
                <button className="m-auto focus:border-gray-400  hover:bg-opacity-30 active:bg-opacity-15 bg-white bg-opacity-20 rounded-lg  border-gray-500 border  ml-2 py-[5px] text-[15px] px-[12px]  w-[30%]">
                  max
                </button>
              </div>
            )}
          ...
          <div className="mt-3 grid px-0">
            <div className="flex">
              {isNFT ? (
                <select
                  className="bg-transparent w-full  py-3 text-center rounded-lg focus:border-gray-400 border-gray-500 border  outline-none "
                  value={boostNFTNumber}
                  onChange={(e) => {
                    setBoostNFTNumber(Number(e.target.value));
                  }}
                >
                  <option className="bg-[#0e1238] py-2 bg-opacity-90" value={1}>
                    Boost NFT 1
                ...
                <p className="text-[20px] p-2 rounded-lg ">Normal token</p>
              )}
            </div>
            <button
              className="mt-3 hover:bg-opacity-30 active:bg-opacity-15 bg-white bg-opacity-20 rounded-lg m-auto py-[10px] text-[20px] rouned-[10px] w-full"
